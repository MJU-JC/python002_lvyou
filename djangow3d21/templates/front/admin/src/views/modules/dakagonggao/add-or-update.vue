<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="积分" prop="jifen">
          <el-select :disabled="ro.jifen" v-model="ruleForm.jifen" placeholder="请选择积分">
            <el-option
                v-for="(item,index) in jifenOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.jifenOptions = "20".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `dakagonggao/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dakagonggao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dakagonggaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
