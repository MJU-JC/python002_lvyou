<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="扣除日期" prop="kouchuriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.kouchuriqi" 
                type="date"
                :readonly="ro.kouchuriqi"
                placeholder="扣除日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.kouchuriqi" label="扣除日期" prop="kouchuriqi">
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="扣除原因" prop="kouchuyuanyin">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="扣除原因"
                  v-model="ruleForm.kouchuyuanyin" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.kouchuyuanyin" label="扣除原因" prop="kouchuyuanyin">
                    <span>{{ruleForm.kouchuyuanyin}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.kouchuriqi = this.getCurDate()
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jifen = true;
          ...
          if(o=='kouchuyuanyin'){
            this.ruleForm.kouchuyuanyin = obj[o];
	    this.ro.kouchuyuanyin = true;
            continue;
          }
          if(o=='kouchuriqi'){
            this.ruleForm.kouchuriqi = obj[o];
	    this.ro.kouchuriqi = true;
            continue;
          }
        }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `koujianjifen/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.koujianjifenCrossAddOrUpdateFlag = false;
						 this.parent.search();
