<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="增加日期" prop="zengjiariqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.zengjiariqi" 
                type="date"
                :readonly="ro.zengjiariqi"
                placeholder="增加日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.zengjiariqi" label="增加日期" prop="zengjiariqi">
              <el-input v-model="ruleForm.zengjiariqi" 
                placeholder="增加日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="增加原因" prop="zengjiayuanyin">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="增加原因"
                  v-model="ruleForm.zengjiayuanyin" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.zengjiayuanyin" label="增加原因" prop="zengjiayuanyin">
                    <span>{{ruleForm.zengjiayuanyin}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.zengjiariqi = this.getCurDate()
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jifen = true;
          ...
          if(o=='zengjiayuanyin'){
            this.ruleForm.zengjiayuanyin = obj[o];
	    this.ro.zengjiayuanyin = true;
            continue;
          }
          if(o=='zengjiariqi'){
            this.ruleForm.zengjiariqi = obj[o];
	    this.ro.zengjiariqi = true;
            continue;
          }
        }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zengjiajifen/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zengjiajifenCrossAddOrUpdateFlag = false;
						 this.parent.search();
